...
async function preloadBalloonImages() {
  ...
  const encryptedBalloonSources = await fetchEncryptedBalloonList();
...
async function fetchEncryptedBalloonList() {
  try {
    const res = await fetch(BALLOON_INDEX_FILE);
    if (!res.ok) throw new Error('Fetch failed');

    const fileList = await res.json();
    
    // If index.json contains paths like "IMG_2559.jpeg.bin", prefix them
    return fileList.map(name => `${BALLOON_FOLDER}/${name}`);
  } catch (err) {
    console.error('❌ Failed to load balloon image list:', err);
    return [];
  }
}
